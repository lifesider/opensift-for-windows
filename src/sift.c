...
static IplImage* convert_to_gray32( IplImage* img )
{
  ...
  gray32 = cvCreateImage( s, IPL_DEPTH_32F, 1 );
  if( img->nChannels == 1 )
    gray8 = cvClone( img );
  else
    {
      gray8 = cvCreateImage( s, IPL_DEPTH_8U, 1 );
			     ...
			     int intvls, double sigma )
{
  ...
  double* sig = malloc((_intvls+3)*sizeof(double));
  int i, o;
  ...
  */
  ...
  for( o = 0; o < octvs; o++ )
	...
	/* blur the current octave's last image to create the next one */
	  ...
	  {
		CvSize s = {gauss_pyr[o][i-1]->width, gauss_pyr[o][i-1]->height};
	    gauss_pyr[o][i] = cvCreateImage( s,
					     IPL_DEPTH_32F, 1 );
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
  ...
  for( o = 0; o < octvs; o++ )
    for( i = 0; i < intvls + 2; i++ )
      {
	CvSize s = {gauss_pyr[o][i]->width, gauss_pyr[o][i]->height};
	dog_pyr[o][i] = cvCreateImage( s,
				       IPL_DEPTH_32F, 1 );
